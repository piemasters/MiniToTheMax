...
const getStopGradients = (paintGradients: PaintGradient[]) => {
  const gradients: JSX.Element[] = [];
  if (paintGradients) {
    for (const g of paintGradients) {
      gradients.push(
        <stop
          offset={`${g.offset}%`}
          stopColor={g.color}
  ...
  if (paintType === 'dry') {
    return (
      <g
        ...
        fill={paintGradients ? '#292929' : getColorByBgColor(paintHex)}
      >
  ...
  if (paintStroke) {
    return (
      <path
        ...
        fill={
          paintGradients
            ? `url(#${paintType}_gradient_${paintHex.substring(1, 7)})`
            : paintHex
        }
        stroke={paintStroke}
        strokeWidth={1}
  ...
  if (paintType === 'shade' && paintName.includes('Gloss')) {
    return (
      <path
        ...
        fill={`url(#${paintType}_gloss_gradient_${paintHex.substring(1, 7)})`}
      />
    );
  }
};
